...
M._lua_tonumber = (L,i) => M._lua_tonumberx(L, i, 0n);
M._lua_tointeger = (L,i) => M._lua_tointegerx(L, i, 0n);
...
M._lua_isnoneornil = (L, n) => M._lua_type(L, n) <= 0n;

M._lua_pushglobaltable = (L) => M._lua_rawgeti(L, M.LUA_REGISTRYINDEX, BigInt(M.LUA_RIDX_GLOBALS));
M._lua_tostring = (L, i) => M._lua_tolstring(L, i, 0n);
M._lua_insert = (L,idx) => { M._lua_rotate(L, idx, 1); };
M._lua_remove = (L,idx) => { M._lua_rotate(L, idx, -1); M._lua_pop(L, 1); };
M._lua_replace = (L,idx) => { M._lua_copy(L, -1n, idx); M._lua_pop(L, 1); };

M._lua_pcall = (L, nargs, nret, msgh) => M._lua_pcallk(L, nargs, nret, msgh, 0n, 0n);
M._luaL_typename = (L,i) => M._lua_typename(L, M._lua_type(L, i));

// lauxlib.h

const wasmstr = s => {
	return BigInt(M.stringToNewUTF8(s));
};
const wasmfunc = (...args) => BigInt(M.addFunction(...args))

M.LUA_LOADED_TABLE = wasmstr('_LOADED');

const str_luaToJs = wasmstr('luaToJs');
const str_jsToLua = wasmstr('jsToLua');
const str_luaWrapObjectMT = wasmstr('luaWrapObjectMT');
const str_luaWrapFuncMT = wasmstr('luaWrapFuncMT');
const str___index = wasmstr('__index');
const str___newindex = wasmstr('__newindex');
const str___tostring = wasmstr('__tostring');
const str___len = wasmstr('__len');
const str___call = wasmstr('__call');
const str__null_ = wasmstr('[null]');
const str_package = wasmstr('package');
const str_loaded = wasmstr('loaded');
const str_global = wasmstr('global');
const str_null = wasmstr('null');
const str_new = wasmstr('new');
const str_tonumber = wasmstr('tonumber');
const str_tostring = wasmstr('tostring');
const str_instanceof = wasmstr('instanceof');
const str_typeof = wasmstr('typeof');
const str_js = wasmstr('js');
const str_ffi = wasmstr('ffi');

// define this before doing any lua<->js stuff
const errHandler = wasmfunc(L => {
	...
	if (msg == 0) {
		if (M._luaL_callmeta(L, 1, str___tostring) &&
			...
			//msg = M._lua_pushfstring(L, wasmstr("(error object is a %s value)"), M._luaL_typename(L, 1));
			msg = M._lua_pushstring(L, wasmstr("(error object is a "+M.UTF8ToString(M._luaL_typename(L, 1))+" value)"));
		}
	}
...
const wrapper___index_func = wasmfunc(L => {
...
const wrapper___newindex_func = wasmfunc(L => {
...
const wrapper___tostring_func = wasmfunc(L => {
	const jsValue = lua_to_js(L, 1);	// optional line or just use the closure variable
	if (jsValue === null) {
	...
	M._lua_pushstring(L, wasmstr(jsValue.toString()));
	return 1;
}, 'ip');

const wrapper___len_func = wasmfunc(L => {
	const jsValue = lua_to_js(L, 1);	// optional line or just use the closure variable
...
console.log("error", e);		
		M._luaL_error(L, wasmstr(e.toString()));
		return 0;
	}
...
const wrapper___call_func = wasmfunc(L => call_func(L, false), 'ip');
const wrapper___callArrow_func = wasmfunc(L => call_func(L, true), 'ip');
...
//console.log('lua_to_js type', t);
	...
	case M.LUA_TSTRING:
		...
		const lenp = M.stackAlloc(8);	// WHY DOESN'T THIS RETURN A 64BIT POINTER?!?!??!!?
		const s = M._lua_tolstring(L, i, BigInt(lenp));
		// convert 'len' ptr from int in mem to number ...
		const len = M.getValue(lenp, 'i64');		// WHY DOESN'T THIS ACCEPT A 64BIT POINTER?!?!?!?!!??
		return M.UTF8ToString(Number(s), Number(len));
	case M.LUA_TTABLE:
...
//console.log('push_js begin top', M._lua_gettop(L));
	...
	case 'bigint':
		...
		return 1;
	case 'string':
		M._lua_pushstring(L, wasmstr(jsValue));
		return 1;
		break;
	case 'function':
	case 'object':
...
const lua = {
	...
	newState : function() {
		...
		//M._luaL_requiref(L, str_ffi, wasmfunc(M._luaopen_ffi, ''), 0);
		...
		//M._luaL_requiref(L, str_ffi, wasmfunc(M._luaopen_ffi, ''), 0);
		const dlhandle = M._dlopen(0n, 257);
		if (!dlhandle) throw "failed to dlopen!";
		const luaopen_ffi = M._dlsym(dlhandle, wasmstr('luaopen_ffi'));
	...
	luaopen_js : function() {
		...
		// not working:
		//M._lua_getfield(L, M.LUA_REGISTRYINDEX, wasmstr(M.LUA_LOADED_TABLE));	// package.loaded
		// instead:
	...
	doString : function(s) {
		...
		const result = M._luaL_loadstring(L, wasmstr(s));	// throw on error?
		if (result != M.LUA_OK) {
